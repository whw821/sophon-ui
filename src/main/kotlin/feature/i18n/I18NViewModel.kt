package feature.i18n

import app.protobuf.Module
import app.protobuf.Project
import cafe.adriel.voyager.core.model.StateScreenModel
import cafe.adriel.voyager.core.model.screenModelScope
import core.Shell.simpleShell
import datastore.i18nDataStore
import datastore.projectDataStore
    ...
    fun importProject(projectFilePath: String?) {
        ...
        screenModelScope.launch {
            ...
            if (structure.isValid) projectDataStore.updateData {
                it.toBuilder().setAbsolutePath(projectFilePath).build()
            }
        ...
        screenModelScope.launch {
            try {
                ...
                i18nDataStore.updateData {
                    it.toBuilder().setToolPath(mutableState.value.toolPath).build()
                }

                // 保存项目路径到DataStore
                projectDataStore.updateData {
                    it.toBuilder().setAbsolutePath(mutableState.value.project.absolutePath).build()
                }
...
private suspend fun String.toProjectStructure(): Project {
    return withContext(Dispatchers.IO) {
        ...
        Project.newBuilder().setAbsolutePath(this@toProjectStructure)
            .addAllModules(structs).build()
    }
}
...
private fun buildModulesWithRes(
    ...
    if (hit) result.add(
        Module.newBuilder().setName(parent.name).setLevel(level)
            .setAbsolutePath(parent.absolutePath).build()
    )
...
val Project.isValid
    get() = absolutePath.isNotBlank() && modulesList.isNotEmpty()

/**
 * 统一的UI状态数据类
 */
data class UiState(
    val project: Project = Project.getDefaultInstance(), // 项目状态
